...
export const ModalSelectInput: React.FC<ModalSelectInputProps> = ({
  ...
  return (
    <div className="space-y-1.5">
      ...
      {/* Selected items displayed as tags at the top */}
      {value.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-2">
          {value.map((item) => {
            ...
            return (
              ...
              >
                <span className="max-w-[150px] truncate" title={displayLabel}>
                  {displayLabel}
                </span>
                <span
                  role="button"
      ...
      <button
        ...
        className="w-full min-h-[46px] bg-secondary-800/80 border border-secondary-600/50 rounded-xl px-4 py-2 flex items-center gap-2 text-left hover:border-primary-500/50 focus:outline-none focus:ring-2 focus:ring-primary-500/50 transition-all group"
      >
        <span className="flex-1 text-secondary-400 text-sm">{placeholder}</span>
